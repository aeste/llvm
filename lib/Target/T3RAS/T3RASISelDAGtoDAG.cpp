...
//===----------------------------------------------------------------------===//
// T3RASDAGToDAGISel - T3RAS specific code to select T3RAS machine
// instructions for SelectionDAG operations.
//===----------------------------------------------------------------------===//
namespace {

class T3RASDAGToDAGISel : public SelectionDAGISel {
...
public:
  explicit T3RASDAGToDAGISel(T3RASTargetMachine &tm) :
  SelectionDAGISel(tm),
  TM(tm), Subtarget(tm.getSubtarget<T3RASSubtarget>()) {}

  // Pass Name
  virtual const char *getPassName() const {
    return "T3RAS DAG->DAG Pattern Instruction Selection";
  }
private:
  // Include the pieces autogenerated from the target description.
  #include "T3RASGenDAGISel.inc"

  /// getTargetMachine - Return a reference to the TargetMachine, casted
  /// to the target-specific type.
  const T3RASTargetMachine &getTargetMachine() {
    return static_cast<const T3RASTargetMachine &>(TM);
  }

  /// getInstrInfo - Return a reference to the TargetInstrInfo, casted
  /// to the target-specific type.
  const T3RASInstrInfo *getInstrInfo() {
...
SelectAddrRegImm(SDValue N, SDValue &Base, SDValue &Disp) {
  ...
  if (N.getOpcode() == ISD::ADD || N.getOpcode() == ISD::OR) {
    ...
    Base = CurDAG->getRegister(T3RAS::R0, CN->getValueType(0));
    return true;
  }
...
SDNode *T3RASDAGToDAGISel::getGlobalBaseReg() {
...
SDNode* T3RASDAGToDAGISel::Select(SDNode *Node) {
  ...
  switch (Opcode) {
    ...
    case T3RASISD::JmpLink: {
      if (TM.getRelocationModel() == Reloc::PIC_) {
        SDValue Chain  = Node->getOperand(0);
        SDValue Callee = Node->getOperand(1);
        SDValue R20Reg = CurDAG->getRegister(T3RAS::R20, MVT::i32);
        SDValue InFlag(0, 0);
            ...
            (isa<ExternalSymbolSDNode>(Callee)))
        {
          ...
          SDValue GPReg = CurDAG->getRegister(T3RAS::R15, MVT::i32);

          // Use load to get GOT target
          SDValue Ops[] = { Callee, GPReg, Chain };
          SDValue Load = SDValue(CurDAG->getMachineNode(T3RAS::LW, dl,
        ...
        SDNode *ResNode = CurDAG->getMachineNode(T3RAS::BRLID, dl, MVT::Other,
...
FunctionPass *llvm::createT3RASISelDag(T3RASTargetMachine &TM) {
  return new T3RASDAGToDAGISel(TM);
}

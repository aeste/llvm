...
namespace {
class T3RASMCCodeEmitter : public MCCodeEmitter {
  T3RASMCCodeEmitter(const T3RASMCCodeEmitter &); // DO NOT IMPLEMENT
  void operator=(const T3RASMCCodeEmitter &); // DO NOT IMPLEMENT
  const MCInstrInfo &MCII;

public:
  T3RASMCCodeEmitter(const MCInstrInfo &mcii, const MCSubtargetInfo &sti,
                      MCContext &ctx)
    : MCII(mcii) {
  }
  ...
  static unsigned GetT3RASRegNum(const MCOperand &MO) {
    // FIXME: getMBlazeRegisterNumbering() is sufficient?
    llvm_unreachable("T3RASMCCodeEmitter::GetT3RASRegNum() not yet "
                     "implemented.");
  }
...
MCCodeEmitter *llvm::createT3RASMCCodeEmitter(const MCInstrInfo &MCII,
                                               const MCSubtargetInfo &STI,
                                               MCContext &Ctx) {
  return new T3RASMCCodeEmitter(MCII, STI, Ctx);
}

/// getMachineOpValue - Return binary encoding of operand. If the machine
/// operand requires relocation, record the relocation and return zero.
unsigned T3RASMCCodeEmitter::getMachineOpValue(const MCInst &MI,
                                             const MCOperand &MO) const {
  if (MO.isReg())
    return getT3RASRegisterNumbering(MO.getReg());
  if (MO.isImm())
...
#define T3RASCodeEmitter T3RASMCCodeEmitter
#define MachineInstr MCInst
#include "T3RASGenCodeEmitter.inc"
#undef T3RASCodeEmitter
#undef MachineInstr
